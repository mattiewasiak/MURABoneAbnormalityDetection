...
np.random.seed(24)
data = ImageDataBunch.from_folder('../data/MURA-v1.1/data2/', ds_tfms=get_transforms(do_flip=True, max_rotate=30, max_warp=0.0, p_lighting=0, xtra_tfms=[]), size=size, bs=bs).normalize(imagenet_stats)


kappa = KappaScore()
kappa.weights = "quadratic"

learner = cnn_learner(data, models.inception_v3, metrics=[error_rate, accuracy, kappa], wd=0.1, model_dir="./models/").to_fp32()
learner.unfreeze()
learner.fit_one_cycle(15, callbacks=[ShowGraph(learner) ,SaveModelCallback(learner)])

learner.save('inception')