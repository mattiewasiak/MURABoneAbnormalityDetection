...
def run_model_on_predictions(bone_name, model):
    ...
    src = (ImageList.from_df(df=df, path='../data/MURA-v1.1/data2/', folder=None, suffix='', cols=0)
                .split_from_df("is_valid")
                .label_from_df(label_delim=None, cols=1))
    data = create_from_ll(src, size = size, bs = bs).normalize(imagenet_stats)
    
    
    classified = ImageList.from_df(df=df, path='../data/MURA-v1.1/data2/', folder=None, suffix='', cols=0)
